...
public class ContinuousRulesStrategy extends SROIQStrategy {
	...
	public void addEdge(Individual subj, Role pred, Node obj, DependencySet ds) {
		...
		if (!merging && !abox.isClosed() && subj.isRootNominal()
				&& obj.isRootNominal()) {
			if (interpreter != null) {
				EdgeList edges = subj.getRNeighborEdges(pred, obj);
				for (Edge edge : edges) {
					if (edge.getFrom().isRootNominal()
							&& edge.getTo().isRootNominal())
						runRules |= interpreter.rete.addFact(edge);
				}

				runRules |= interpreter.rete.addDifferents(subj);
				if (obj.isIndividual())
					runRules |= interpreter.rete
							.addDifferents((Individual) obj);
			}
	...
	private void applyFact(Fact fact) {
		...
		if (fact.getElements().size() == 3) {
			...
			Individual subj = abox.getIndividual(fact.getElements().get(
					Compiler.SUBJ));
			if (subj.isMerged()) {
				ds = ds.union(subj.getMergeDependency(true),
						abox.doExplanation());
				subj = subj.getSame();
			}
	...
	public void applyRete() {
		...
		for (Fact fact : inferred) {
			...
			if (fact.getElements().size() > 3
					&& fact.getElements().get(Compiler.PRED)
							.equals(ContinuousReteTransformer.VARBINDING)) {
	...
	public void applyRuleBindings() {
		...
		for (Fact ruleBinding : partialBindings.keySet()) {
			Pair<Rule, VariableBinding> pair = continuousTransformer
					.translateFact(ruleBinding);
			Rule rule = pair.first;
			VariableBinding initial = pair.second;

			for (VariableBinding binding : bindingStrategy.createGenerator(
					rule, initial)) {

				Pair<Rule, VariableBinding> ruleKey = new Pair<Rule, VariableBinding>(
						rule, binding);
				if (!rulesApplied.containsKey(ruleKey)) {
					...
					int branch = createDisjunctionsFromBinding(binding, rule,
							ruleBinding.getDependencySet());
	...
	public void complete(Expressivity expr) {
		...
		for (Entry<Rule, Rule> e : abox.getKB().getNormalizedRules().entrySet()) {
			...
			Set<ATermAppl> explain = abox.doExplanation() ? rule
					.getExplanation(atermTranslator) : Collections
					.<ATermAppl> emptySet();
		...
		while (!abox.isComplete()) {
			while (abox.isChanged() && !abox.isClosed()) {
				...
				if (log.isLoggable(Level.FINE)) {
					log.fine("Branch: " + abox.getBranch() + ", Depth: "
							+ abox.stats.treeDepth + ", Size: "
							+ abox.getNodes().size() + ", Mem: "
			...
			if (abox.isClosed()) {
				if (log.isLoggable(Level.FINE))
					log.fine("Clash at Branch (" + abox.getBranch() + ") "
							+ abox.getClash());
				...
				if (PelletOptions.SATURATE_TABLEAU) {
					...
					for (int i = abox.getBranches().size() - 1; i >= 0; i--) {
						...
						unexploredBranch.setTryNext(unexploredBranch
								.getTryNext() + 1);
						if (unexploredBranch.getTryNext() < unexploredBranch
								.getTryCount()) {
							restore(unexploredBranch);
							System.out.println("restoring branch "
									+ unexploredBranch.getBranch()
									+ " tryNext = "
									+ unexploredBranch.getTryNext()
									+ " tryCount = "
	...
	private int createDisjunctionsFromBinding(VariableBinding binding,
			Rule rule, DependencySet ds) {
		...
		if (atoms.isEmpty()) {
			if (rule.getHead().isEmpty()) {
				...
				for (RuleAtom atom : rule.getHead()) {
					ruleAtomAsserter.assertAtom(atom, binding, ds, false, abox,
							this);
				}
			}
			return -1;
		...
		// can directly assert it without creating a disjunction
			...
			ruleAtomAsserter.assertAtom(atoms.get(0), binding, ds, true, abox,
					this);
			return -1;
		} else {
			RuleBranch r = new RuleBranch(abox, this, ruleAtomAsserter, atoms,
					binding, bodyAtomCount, ds);
			addBranch(r);
			r.tryNext();
			return r.getBranch();
		}
	...
	public void mergeTo(Node y, Node z, DependencySet ds) {
		...
		if (!abox.isClosed() && (interpreter != null)
				&& (y.isRootNominal() || z.isRootNominal())) {
			if (y.isRootNominal())
	...
	private void restoreRules(Branch branch) {
		...
		for (Iterator<Map.Entry<Pair<Rule, VariableBinding>, Integer>> ruleAppIter = rulesApplied
			...
			Map.Entry<Pair<Rule, VariableBinding>, Integer> ruleBranchEntry = ruleAppIter
					.next();
		...
		for (Iterator<Map.Entry<Fact, Integer>> iter = partialBindings
